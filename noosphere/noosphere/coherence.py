...
from noosphere.models import Principle, Claim, CoherenceReport
...
class NLIEngine:
    ...
    Attempts to load local DeBERTa-based NLI model; falls back to Claude API.
    """
    ...
    def __init__(self):
        ...
        if TRANSFORMERS_AVAILABLE:
            try:
                logger.info("Loading local NLI model (cross-encoder/nli-deberta-v3-base)...")
                ...
                logger.warning(f"Failed to load local NLI model: {e}")
                self.backend = None

        if self.backend is None:
            if ANTHROPIC_AVAILABLE:
                logger.info("Using Claude API for NLI")
                self.client = Anthropic()
                self.use_api = True
            else:
                logger.error("Neither transformers nor anthropic library available")
...
class CoherenceEngine:
    ...
    def _layer_6_llm_judge(self) -> float:
        ...
        if not ANTHROPIC_AVAILABLE:
            logger.warning("Layer 6: Anthropic library not available, skipping")
            return 0.5
...
Respond with ONLY a single floating-point number between 0.0 and 1.0, nothing else."""
    ...
    def _identify_contradictions(self) -> list[tuple[str, str, float]]:
        ...
        Returns:
            List of (id_a, id_b, severity) tuples, sorted by severity
        """
        contradictions = []
        ...
        for i in range(len(self.propositions)):
            for j in range(i + 1, len(self.propositions)):
                ...
                if contradiction_conf > 0.6:  # Threshold
                    contradictions.append((p1.id, p2.id, float(contradiction_conf)))
...
if __name__ == "__main__":
    ...
    print(f"Layer Scores: {report.layer_scores}")
    print(f"Contradictions: {report.contradictions_found}")
    print(f"Weak Links: {report.weakest_links}")
