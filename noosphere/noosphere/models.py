...
# ── Currents (Wave 1): Current-events data model ────────────────────────────
...
class FollowUpMessageRole(str, Enum):
    USER = "user"
    ASSISTANT = "assistant"
    SYSTEM = "system"


class CurrentEvent(StrictModel):
    id: str
...
class EventOpinion(StrictModel):
    id: str
    event_id: str
    generator_model: str
    ...
    sources_cited: int
    generator_tokens_prompt: int
    generator_tokens_completion: int
    revoked: bool = False
    revoked_reason: Optional[str] = None


class OpinionCitation(StrictModel):
    id: str
    opinion_id: str
    ...
    def _xor_source(self) -> "OpinionCitation":
        has_conc = self.conclusion_id is not None
        has_claim = self.claim_id is not None
        if has_conc == has_claim:
            raise ValueError(
                "OpinionCitation requires exactly one of conclusion_id or claim_id"
            )
        return self


class FollowUpSession(StrictModel):
    id: str
    opinion_id: str
    created_at: datetime
