...
- **Docker** — `deploy/docker/theseus-codex/Dockerfile` (Next standalone), `deploy/docker/ingest-worker/Dockerfile` (Node + Python for `processUpload`).
...
```bash
docker build -f deploy/docker/theseus-codex/Dockerfile -t theseus/theseus-codex:dev .
