...
import CurrentsFollowupChat from "@/app/currents/[id]/FollowupChat";
...
type FollowupKind = FollowupStreamEvent["kind"];
...
async function errorFromResponse(response: Response): Promise<FollowupStreamError> {
  ...
  return new FollowupStreamError(
    payloadMessage(payload) ?? `Follow-up request failed with HTTP ${response.status}`,
    {
...
export async function* streamForecastFollowup(
  predictionId: string,
  question: string,
  sessionId: string | null,
): AsyncGenerator<FollowupStreamEvent> {
  const response = await fetch(`/api/forecasts/${encodeURIComponent(predictionId)}/follow-up`, {
    method: "POST",
    headers: {
...
function toCurrentsSource(source: PublicForecastSource): PublicSource {
  return {
    id: source.id,
    opinion_id: source.prediction_id,
    source_kind: source.source_type.toLowerCase(),
    source_id: source.source_id,
    source_text: source.source_text,
    quoted_span: source.quoted_span,
    retrieval_score: source.retrieval_score ?? 0,
    is_revoked: source.is_revoked,
    revoked_reason: source.revoked_reason,
    canonical_path: source.canonical_path,
  };
}

const tokenBridgeStyle = {
  "--currents-bg-elevated": "var(--forecasts-bg-elevated)",
  "--currents-border": "var(--forecasts-border)",
  "--currents-parchment": "var(--forecasts-parchment)",
  "--currents-parchment-dim": "var(--forecasts-parchment-dim)",
  "--currents-muted": "var(--forecasts-muted)",
  "--currents-gold": "var(--forecasts-cool-gold)",
  "--currents-amber": "var(--forecasts-cool-gold)",
} as CSSProperties;

export default function ChatPanel({ predictionId, sources }: ChatPanelProps) {
  const adaptedSources = useMemo(
    () => sources.map((source) => toCurrentsSource(source)),
    [sources],
  );

  return (
    <div style={tokenBridgeStyle}>
      <CurrentsFollowupChat
        opinionId={predictionId}
        sources={adaptedSources}
        stream={streamForecastFollowup}
      />
    </div>
  );
}
