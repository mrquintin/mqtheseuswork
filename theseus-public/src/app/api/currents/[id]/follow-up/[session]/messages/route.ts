...
async function resolveParams(
  ...
  if (raw && typeof (raw as { then?: unknown }).then === "function") {
    return await (raw as Promise<{ id: string; session: string }>);
  }
  return raw as { id: string; session: string };
}

export async function GET(req: Request, ctx: Ctx) {
  const params = await resolveParams(ctx);
  const backend = new URL(
    `/v1/currents/${encodeURIComponent(params.id)}/follow-up/${encodeURIComponent(params.session)}/messages`,
    BACKEND,
  );
