...
Theseus Codex      & electron-builder & \file{.app} in DMG        & \file{.exe} via NSIS installer \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.2em]
  ...
  \item Node.js 20+ with npm (for Theseus Codex).
...
\file{dialectic/dialectic/updater.py} runs a background thread on application launch that fetches a JSON manifest from a configurable URL (default: \cmd{https://github.com/mrquintin/mqtheseuswork/releases/download/latest-main/dialectic-latest.json}). The rolling release workflow publishes this manifest alongside \file{Dialectic.dmg} and \file{Dialectic-Setup.exe}. The manifest format is:
\begin{lstlisting}
{
  "version": "0.2.0",
  "commit": "abc123...",
...
If the manifest version is newer than the running version, or the version is unchanged but the release commit/build differs from the build embedded in the app, the callback notifies the user in the Qt dashboard. The check is non-blocking (10s timeout), daemon-threaded, and silently no-ops on network errors. There is no silent auto-install; the user must download and install manually. The default manifest URL can be overridden with \cmd{DIALECTIC_UPDATE_MANIFEST_URL}.
...
% =======================================================================
\section{Theseus Codex (Electron)}
% =======================================================================
...
\textbf{File} & \textbf{Purpose} \\
\midrule
\file{theseus-codex/desktop/main.ts}          & Electron main process: lifecycle, window, migrations. \\
\file{theseus-codex/desktop/next-server.ts}    & Starts the Next.js standalone server as a child process. \\
\file{theseus-codex/desktop/db-path.ts}        & Resolves SQLite database to platform app-data directory. \\
\file{theseus-codex/desktop/preload.ts}        & Context bridge (platform info, deep-link IPC). \\
\file{theseus-codex/desktop/updater.ts}        & \cmd{electron-updater} integration for auto-update. \\
\file{theseus-codex/desktop/dev.ts}            & Dev mode: starts Next.js dev server + Electron. \\
\file{theseus-codex/electron-builder.yml}      & Build config: DMG, NSIS, signing, notarization, publish. \\
\file{theseus-codex/assets/entitlements.mac.plist} & macOS entitlements for hardened runtime. \\
\file{theseus-codex/scripts/generate\_icons.js}    & Generates \file{.icns} and \file{.ico} from source PNG. \\
\file{theseus-codex/scripts/build\_desktop\_macos.sh} & Full macOS desktop build. \\
\file{theseus-codex/scripts/build\_desktop\_windows.ps1} & Full Windows desktop build. \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[leftmargin=1.5em,itemsep=0.1em]
  ...
  \item Resolves the SQLite database path via \file{db-path.ts}: \file{userData/data/theseus-codex.db} (platform-specific --- \file{\textasciitilde/Library/Application Support/Theseus Codex/} on macOS, \file{\%APPDATA\%/Theseus Codex/} on Windows).
...
Output: \file{theseus-codex/dist-electron/Theseus Codex-\{version\}.dmg}.
...
Output: \file{theseus-codex\textbackslash dist-electron\textbackslash Theseus Codex Setup \{version\}.exe}.
...
The Theseus Codex uses \cmd{electron-updater} for automatic updates via GitHub Releases. The \file{electron-builder.yml} contains a \cmd{publish} block:
\begin{lstlisting}
publish:
  provider: github
