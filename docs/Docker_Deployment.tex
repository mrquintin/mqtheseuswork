...
\midrule
Founder Portal   & node:20-alpine & 3000 & Next.js 16 standalone server with Prisma 7 and SQLite. \\
...
\file{founder-portal/Dockerfile} uses a three-stage build:
...
The entrypoint script (\file{scripts/docker-entrypoint.sh}) checks whether \file{/app/data/founder-portal.db} exists. If not (first launch), it copies the seed database from the builder stage. Then it executes the Next.js standalone server.

Key environment variables:
\begin{itemize}[leftmargin=1.5em,itemsep=0.1em]
  \item \cmd{DATABASE\_URL} --- defaults to \cmd{file:/app/data/founder-portal.db}. Override to point at a Postgres URL for scaled deployments.
...
# View logs
docker compose logs -f founder-portal
...
After startup, the Founder Portal is at \cmd{http://localhost:3000} and the Public site at \cmd{http://localhost:8080}.
...
# Founder Portal
cd founder-portal
docker build -t theseus/founder-portal:latest .
...
Build times are approximately 2--4 minutes for the Founder Portal (npm install + Next.js build + Prisma generate) and 1--2 minutes for Theseus Public (npm install + static export).
...
docker buildx build --platform linux/amd64,linux/arm64 \
  -t theseus/founder-portal:latest \
  --push \
  ./founder-portal
\end{lstlisting}
...
\midrule
Founder Portal & 200--250\,MB & node:20-alpine base + Next.js standalone + Prisma client + better-sqlite3. \\
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.1em]
  \item \textbf{Portal Deployment} --- the Founder Portal container with configurable replicas, resource limits, and a PersistentVolumeClaim for the SQLite database (or an environment variable pointing to an external PostgreSQL).
...
The Founder Portal uses SQLite via Prisma's \cmd{better-sqlite3} adapter. In Docker, the database file lives on a named volume (\file{/app/data/founder-portal.db}). This is appropriate for single-server deployments and local development.
...
For SQLite: back up the volume-mounted \file{founder-portal.db} file. Since SQLite uses write-ahead logging, use \cmd{sqlite3 /app/data/founder-portal.db ".backup /backup/founder-portal.db"} for a consistent copy while the server is running.
...
\textbf{``EACCES: permission denied'' on startup.} The non-root user (\cmd{nextjs}, UID 1001) must own the data directory. If the volume was previously written by root, fix permissions: \cmd{docker compose exec founder-portal chown -R nextjs:nodejs /app/data}.
