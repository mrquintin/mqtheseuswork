...
\textbf{Distribution.} The Theseus Codex ships as both a web service and a desktop application.
...
\file{Product\_Description.pdf}          & How the three user-facing softwares (Noosphere, Dialectic, Theseus Codex) fit together. The integration narrative. \\
...
\begin{enumerate}[leftmargin=1.5em,itemsep=0.3em]
  ...
  \item \textbf{Ingest.} A founder uploads the file via the Theseus Codex or runs \texttt{python -m noosphere ingest}. The ingester parses the file, extracts frontmatter, segments into chunks on paragraph or speaker-turn boundaries, and writes \texttt{Artifact} and \texttt{Chunk} rows. Content-addressed IDs make the ingest idempotent.
...
\subsection{Electron desktop app (Theseus Codex)}

The Theseus Codex's Electron wrapper comprises six TypeScript modules in \file{desktop/}:
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.1em]
  ...
  \item \file{db-path.ts} --- resolves SQLite to the platform-appropriate app-data directory (\file{userData/data/theseus-codex.db}).
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.1em]
  \item \file{theseus-codex/Dockerfile} --- multi-stage build: dependency install, Next.js build with Prisma generation, standalone extraction, seed database from pre-run migrations, \texttt{node:20-alpine} runner with volume-mounted SQLite.
  ...
  \item \file{docker-compose.yml} (repo root) --- Theseus Codex on port 3000, Public on port 8080, shared \texttt{portal-data} volume for SQLite persistence.
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.1em]
  ...
  \item \file{build-theseus-codex.yml} --- matrix build for Electron. Node 20 setup, Next.js build, TypeScript compilation, icon generation, \texttt{electron-builder} with platform flag. Code signing via certificate secrets (macOS: P12 base64 + password; Windows: PFX base64 + password).
  \item \file{build-docker.yml} --- builds Docker images for Theseus Codex and Theseus Public using Buildx with GitHub Actions cache. Validates \texttt{docker-compose.yml}.
...
\file{.github/dependabot.yml} configures weekly dependency updates for npm (Theseus Codex, Public), pip (Noosphere, Dialectic), and GitHub Actions.
